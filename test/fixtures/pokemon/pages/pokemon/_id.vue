<template>
<div>
  <div class="bg-green-50 rounded p-5 w-500px mx-auto text-center">
  ...
  <div class="grid grid-cols-10 gap-8 w-full">
    <nuxt-link v-for="(p, key) in pokemons" :key="key" v-if="key < 21 && p.id !== pokemon.id" :to="'/pokemon/' + p.id" class="bg-white rounded p-3 flex items-center justify-center flex-col">
      <img :src="'/thumbnails/'  + p.id.toString().padStart(3, '0') + '.webp'" width="50" height="50" class="mb-2" >
      <h2 class="text-gray-800">{{ p.name.english }}</h2>
    </nuxt-link>
  </div>
</div>
</template>
...
export default {
  async asyncData({ payload, params }) {
    ...
    return {
      pokemon: pokemons.find(p => parseInt(p.id) === parseInt(params.id)),
      pokemons
    }
  }
}
